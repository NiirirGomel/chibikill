...
procedure glTgaAlfaON;
procedure glTgaAlfaOFF;

procedure glPreTex_LoAD;
procedure glMenu_LoAD;
procedure glPreTex_Free;
procedure glMenu_Free; 

var
  TexChar: GLuint;                            // Текстура символов
  TexLoadScreen: array [0..1] of GLuint;      // Загрузочный Экран
  TexMenu: array [0..12] of GLuint;            // Меню
...
procedure glResizeWnd2D(Width, Height : Integer);
begin
  ...
  gluPerspective(0, Width/Height, 0, 0);  // Do the perspective calculations. Last value = max clipping depth
...
procedure glPreTex_LoAD;
begin
  LoadTexture('data/fon.bmp', TexLoadScreen[0], false);    // Load the Texture
  LoadTexture('data/fon_app.tga',TexLoadScreen[1],false);
  LoadTexture('data/CharAr.tga',TexChar,false);
end;

procedure glMenu_LoAD;
begin
  LoadTexture('data/menu/bg.bmp', TexMenu[0], false);    // Load the Texture
  LoadTexture('data/menu/start.tga',TexMenu[1],false);
  LoadTexture('data/menu/seting.tga',TexMenu[2],false);
  LoadTexture('data/menu/score.tga',TexMenu[3],false);
  LoadTexture('data/menu/exit.tga',TexMenu[4],false);
  LoadTexture('data/menu/help.tga',TexMenu[5],false);
  LoadTexture('data/menu/score_bg.bmp',TexMenu[7],false);
  LoadTexture('data/menu/newscore_bg.bmp',TexMenu[8],false);
  LoadTexture('data/menu/help_bg.bmp',TexMenu[9],false);
  LoadTexture('data/menu/seting_bg.bmp',TexMenu[10],false);
  LoadTexture('data/menu/seting_on.tga',TexMenu[11],false);
  LoadTexture('data/menu/seting_off.tga',TexMenu[12],false);
end;

procedure glPreTex_Free;
begin
  glDeleteTextures(1, TexLoadScreen[0]);    // Free the Texture
  glDeleteTextures(1, TexLoadScreen[1]);    // Free the Texture
  glDeleteTextures(1,TexChar);
end;

procedure glMenu_Free;                  // Free the Texture
var
  n: Integer;
begin
  for n:=0 to Length(TexMenu) - 1 do
    glDeleteTextures(1, TexMenu[n]);
end;
